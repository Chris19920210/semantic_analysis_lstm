...
def load_data(max_len,batch_size,n_words=20000,valid_portion=0.1,sort_by_len=True):
    ...
    def padding_and_generate_mask(x,y,new_x,new_y,new_mask_x):
        ...
        for i,(x,y) in enumerate(zip(x,y)):
            ...
            if len(x)<=max_len:
                # insert xth-sentence to ith row, row size is bounded by maxlen
                new_x[i,0:len(x)]=x
                # insert xth-sentence to itt column, column size is bounded by maxlen
...
def batch_iter(data,batch_size):
    ...
    # x row-> sentence number column -> word
    # y row -> label
    # mask row -> word column -> sentence
    x,y,mask_x=data
    x=np.array(x)
    y=np.array(y)
    data_size=len(x)
